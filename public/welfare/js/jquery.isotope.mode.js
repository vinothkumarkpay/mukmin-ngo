			...
			success : function (data) { 
				if (data !== '') {
					...
					if (data.slice(0, 6) === 'finish') {
						newHTML = jQuery(data.replace(/cmsms_post_timeline/g, 'cmsms_post_timeline shortcode_animated').replace(/cmsms_post_masonry/g, 'cmsms_post_masonry shortcode_animated').replace(/cmsms_post_puzzle/g, 'cmsms_post_puzzle shortcode_animated').slice(6, -1));
						
						
						blogContainer.find('.cmsms_wrap_more_items').hide();
					} else {
						newHTML = jQuery(data.replace(/cmsms_post_timeline/g, 'cmsms_post_timeline shortcode_animated').replace(/cmsms_post_masonry/g, 'cmsms_post_masonry shortcode_animated').replace(/cmsms_post_puzzle/g, 'cmsms_post_puzzle shortcode_animated'));
...
function startGallery(id, gallery_type, gallery_count, gallery_arr) { 
	...
	if (gallery_count !== 'false' && gallery_arr !== 'false') {		
		galleryContainer.find('.cmsms_gallery_items_loader').on('click', function () {
			galleryContainer.find('.cmsms_wrap_more_items').addClass('cmsms_loading cmsms_theme_icon_spinner').find('.cmsms_wrap_items_loader').slideUp('fast');
			...
			if (gallery_arr.length == 0) {
				galleryContainer.find('.cmsms_wrap_more_items').hide();
			} else {
				galleryContainer.find('.cmsms_wrap_more_items').removeClass('cmsms_loading cmsms_theme_icon_spinner').find('.cmsms_wrap_items_loader').slideDown('fast');
 			}
		} );
	}
